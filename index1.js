...
const accountsPath = NODE_ENV === 'development' ? path.join(__dirname, 'src', 'accounts') : app.getPath('userData')
const sessionsPath = path.join(__dirname, 'src', 'sessions')
const filesPath = NODE_ENV === 'development' ? path.join(__dirname, 'src', 'files') : app.getPath('temp')
...
const loginHandler = async (req, res) => {
    const textData = fs.readFileSync(path.join(accountsPath, req.body.account + '.data.json'), {'encoding': 'utf8', 'flag':'r'});
    let data;
...
const registerHandler = (req, res) => {
    ...
    fs.writeFileSync(path.join(accountsPath, `${name}.data.json`), JSON.stringify(req.body, null, 2));
...
const removeHandler = (req, res) => {
    ...
    fs.rmdir(path.join(accountsPath, name), { recursive: true }, (err) => {
        if (err) {
            throw err;
        }

        console.log(`${path.join(accountsPath, name)} is deleted!`);
    });
